  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        <Sidebar />
        <main style={{ flex: 1, padding: '20px', overflowY: 'auto', background: 'var(--bg)', minHeight: 'calc(100vh - 52px)' }}>
          <ActiveComponent />
        </main>
      </div>
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--bg-2)', borderTop: '1px solid var(--border)', padding: '5px 20px', display: 'flex', alignItems: 'center', gap: 16, fontSize: 11, color: 'var(--text-3)', zIndex: 40 }}>
        ...
        <span style={{ marginLeft: 'auto', fontFamily: 'monospace' }}>TrustGuard AI © {new Date().getFullYear()}</span>
      </div>
    </div>
  );
}
