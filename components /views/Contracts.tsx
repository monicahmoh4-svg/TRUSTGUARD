...
import {
  ...
  ScoreRing, ScanBar, SignalItem, Badge, Spinner, EmptyState,
} from '@/components/ui';
...
interface ContractResult {
  ...
  tokenMetrics: {
    ...
    topHolders?: Array<{ address: string; quantity: string }>;
    topHolderPct?: number;
  };
  aiAnalysis: {
    summary: string;
...
export default function Contracts() {
  ...
  const badgeVariant = score
    ? (score.total >= 65 ? 'danger' : score.total >= 35 ? 'warning' : 'safe') as 'danger' | 'warning' | 'safe'
    : 'default' as 'default';
  ...
  return (
    <div className="animate-fade-in">
      <SectionHeader
        ...
        subtitle="Detect rug pulls, honeypots, ownership risks, and exploit vectors using real on-chain data"
      />
      ...
      {result && score && ai && (
        <div className="animate-slide-in space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <Card>
              ...
              <div className="text-center py-4">
                <div className="text-[40px] font-bold font-mono-custom" style={{ color: scoreColor }}>
                  {score.total}
                </div>
                <div className="text-[11px] text-[var(--text-2)] uppercase tracking-wide mb-3">Risk Score</div>
                <div
                  className="inline-block px-4 py-1.5 rounded-full font-bold text-[13px] tracking-wider"
                  style={{
            ...
            <Card className="col-span-2">
              <CardHeader>
                ...
                <Badge variant={result.contract.isVerified ? 'safe' : 'danger'}>
                  {result.contract.isVerified ? '✓ Verified' : '✗ Unverified'}
                </Badge>
              </CardHeader>
              <p className="text-[13px] text-[var(--text-2)] leading-relaxed mb-3">{ai.summary}</p>
              <div
                className="p-2.5 rounded-[6px] text-[12px] font-medium mb-3"
                style={{ background: `${scoreColor}11`, color: scoreColor }}
              >
                → {ai.recommendation}
              </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {ai.criticalRisks.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <Badge variant="danger">{ai.criticalRisks.length}</Badge>
                </CardHeader>
                {ai.criticalRisks.map((r, i) => (
                  <div key={i} className="flex gap-2 py-2 border-b border-[var(--border)] last:border-0 text-[12px]">
                    <span className="text-[var(--red)] flex-shrink-0">▲</span>
                    <span className="text-[var(--text-2)]">{r}</span>
                  </div>
                ))}
                {ai.criticalRisks.length === 0 && (
                  <p className="text-[12px] text-[var(--text-3)]">No critical risks detected</p>
                )}
              </Card>
            )}
            ...
            <Card>
              <CardHeader>
                ...
                <Badge variant="warning">{ai.warnings.length}</Badge>
              </CardHeader>
              {ai.warnings.map((w, i) => (
                <div key={i} className="flex gap-2 py-2 border-b border-[var(--border)] last:border-0 text-[12px]">
                  <span className="text-[var(--amber)] flex-shrink-0">●</span>
                  <span className="text-[var(--text-2)]">{w}</span>
                </div>
              ))}
            ...
            <Card>
              <CardHeader>
                ...
                <Badge variant="safe">{ai.positiveSignals.length}</Badge>
              </CardHeader>
              {ai.positiveSignals.map((p, i) => (
                <div key={i} className="flex gap-2 py-2 border-b border-[var(--border)] last:border-0 text-[12px]">
          ...
          <Card>
            <CardHeader>
              <CardTitle>All Risk Engine Signals ({score.signals.length})</CardTitle>
            </CardHeader>
      ...
      {!result && !loading && !error && (
        <EmptyState
          ...
          description="Paste any ERC-20, DeFi, or NFT contract address to get an instant security audit"
        />
      )}
    </div>
  );
